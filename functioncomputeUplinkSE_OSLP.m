...
for iRealz = 1:numRealz
    ...
    P = Q; %Initialize P, Eq. (31)
    
    T = (P*Hhat_1')/(Sigma + Hhat_1*P*Hhat_1'); % Eq. (30)
    P = (eye(K) - T*Hhat_1)*P; % Eq. (31)
    ...
    for l = 2:L
        ...
        Hhat  = H_hat((l-1)*N+1:l*N,:,iRealz) ; % Collecting channel estimate of AP l
        Sigma = sum(R_tilde(:,:,:,l).*powUEs,3) + eye(N);
        
        T = (P*Hhat')/(Sigma + Hhat*P*Hhat'); % Eq. (30)
        
        V = [(eye(K) - T*Hhat)*V,T]; % Eq. (36)
