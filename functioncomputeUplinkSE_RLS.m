function SE = functioncomputeUplinkSE_RLS(H_hat,R_tilde,tau_c,tau_p,numRealz,K,L,N,allocatedPowUEs)
...
for iRealz = 1:numRealz
    ...
    T = T - (T*conj(Hhat_1)*transpose(Hhat_1)*T)/(1+transpose(Hhat_1)*T*conj(Hhat_1));
    ...
    for l = 2:L
        ...
        T = T - (T*conj(Hhat_2)*transpose(Hhat_2)*T)/(1+transpose(Hhat_2)*T*conj(Hhat_2));
        
        A = (eye(K) - T*conj(Hhat_2)*transpose(Hhat_2));
        B = T*conj(Hhat_2);
